    function requestComments(url) {
        $.getJSON( url, function( result ) {
            ...
            for (var i = 0; i < data.length; i++) {
                createCommentBlock(data[i].user, data[i].date, data[i].text, data[i].plus, data[i].minus, data[i].id, data[i].parent, data[i].hasOpinion, false);
            }
    ...
    function createCommentBlock(nameComment, dateComment, textComment, plus, minus, commentId, parentId, hasOpinion, isNew) {
        var commentHtml='';
    ...
    function addComment(text, newsId, parentId, $button, isNew) {
    if(!parentId) {
        parentId = 0;
    }
   // console.log('/add_comment/' + newsId+'/'+ parseInt(parentId));
        $.post('/add_comment/' + newsId+'/'+ parseInt(parentId), {request: text})
            .done(function (data) {
                $button
                    .removeClass('fa-spinner')
                    .removeClass('fa-spin')
                ...
                if(parentId){
                    commentFieldBackNormal();
                }

                if($('#js-header').data('category') != 4){
                    var d = new Date();
                    var dd = d.getHours() + ':' + d.getMinutes() + ' ' + d.getDay() + '/' + d.getMonth() + '/' + d.getFullYear();
                    createCommentBlock($('#logout').data('user_name'), dd, text, 0, 0, data, parentId, null, isNew);
                }
                else {
    ...
    function updateComment($button, $textarea, commentId) {
    ...
    function updatePlus(plusId, $elementForChange) {
        $.ajax({
            type: "GET",
    ...
    function updateMinus(minusId, $elementForChange) {
        $.ajax({
            type: "GET",
    ...
    function commentFieldBackNormal() {
    ...
    $('body').on('click', '#comment-add', function (event) {
        ...
        if (text!=='') {
            ...
            $('#button-icon')
                ...
                .addClass('fa-spinner')
                .addClass('fa-spin');
            addComment(text, newsId, null, $('#button-icon'), true);
    ...
    $('body').on('click', '.js-plus', function (event) {
        ...
        updatePlus($(this).closest('.comment-wrap').attr('id').slice(8), $(this).find('i'));
    });
    ...
    $('body').on('click', '.js-minus', function (event) {
        ...
        updateMinus($(this).closest('.comment-wrap').attr('id').slice(8), $(this).find('i'));
    });
    ...
    $('body').on('click', '#comment-edit', function (event) {
        ...
        updateComment($(this), $textArea, commentId);
    });
